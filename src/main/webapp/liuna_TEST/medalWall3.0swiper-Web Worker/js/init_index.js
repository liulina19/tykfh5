$(document).ready(function() {
				...
				if(data.resCode == '200' && data.activityList.length > 0) {
					...
					$.each(data.activityList, function(i, item) {
						...
						tpl.push('<div class="clic1 swiper-slide" id="Activity' + i + '">' +
							'<img src="' + item.activityImgURL + '?v=2" class="imgclass1" />' +
							'</div>');
	...
	function userInformation(datas) {
		...
		//用户星级
		if(datas.membershipLevel > 0) {
			$('.star').html(starLv(datas.membershipLevel));
		} else if(datas.membershipLevel == 0) {
			$('.xx').html('普通用户');
			$('.xx').css('width','21%');
		} else {
			$('.xx').html('数据异常');
		...
		if(datas.growthpoint != undefined && datas.growthpoint != null && datas.growthpoint != "") {
			$('.czzdata').html(datas.growthpoint);
		} else {
			$('.czzvulue').html('异常数据');
		}
	...
	var growthPointShow = function(fd) {
		if((starLv(userInfo.userLevel) == '6')) {
			$('.czzqj').html(fd[0].starvalue);
		} else if(starLv(userInfo.userLevel) == '5') {
			$('.czzqj').html(fd[1].starvalue);
		} else if(starLv(userInfo.userLevel) == '4') {
			$('.czzqj').html(fd[2].starvalue);
		} else if(starLv(userInfo.userLevel) == '3') {
			$('.czzqj').html(fd[3].starvalue);
		} else if(starLv(userInfo.userLevel) == '2') {
			$('.czzqj').html(fd[4].starvalue);
		} else if((starLv(userInfo.userLevel) == '1') || starLv(userInfo.userLevel) == '0') {
			$('.czzqj').html(fd[5].starvalue);
		} else if((starLv(userInfo.userLevel) == '7')) {
			$('.czzqj').html('8000以上');
		}
...
// 创建new Worker   并引入workerVoucher.js
    function startWorkerVoucher(){
        if(typeof(Worker)!=="undefined"){
            return new Worker("js/workerVoucher.js");
        }else{
            return null;
        }
    }
	...
	function voucher() {
        ...
        var urlParamVoucher = encodingRedirectUrlForClientUni('/clientuni/services/userManage/GetVoucher?' +
            'reqParam={"transactionId":"' + transactionId + '","channelCode":"H5002018","mobile":"' + userInfo.mobile + '","sign":"' + sign + '"}');
        v.postMessage(urlParamVoucher);
        v.onmessage = function(e){
             var worderVoucherData = (JSON.parse(e.data));
            // alert(e.data);
            if(worderVoucherData.resCode == 200 || worderVoucherData.resCode == '200') {
                //当前可用积分
                if(pointValue != '' && pointValue != null && typeof pointValue != 'undefined') {
                                        //isNaN() 函数可用于判断其参数是否是 NaN，该值表示一个非法的数字
                    $('#vouchers').html(isNaN(parseInt(worderVoucherData.voucherVal)) ? '暂无数据' : worderVoucherData.voucherVal)
                } else {
                    $('#vouchers').html('暂无数据');
                }
            }
...
// 创建new Worker   并引入workerCurjf.js(可用积分)
    function startWorker(){
        if(typeof(Worker)!=="undefined"){
             return new Worker("js/workerCurjf.js");
        }else{
            return null;
        }
    }
	...
	function curjf() {
       ...
       var urlParam= encodingRedirectUrlForClientUni('/clientuni/services/starLevel/custStarLevelQuery?reqParam={"transactionId":"' + transactionId + '","clientNbr":"' + clientnbr + '","channelCode":"' + channelCode + '","deviceType":"1","prvince":"' + encodeURIComponent(encodeURIComponent(userInfo.province)) + '","sign":"' + sign + '"}');
       w.postMessage(urlParam);

        w.onmessage=function(e){
            // alert((JSON.parse(e.data)).resCode);
            //将传送过来的数据(数据类型是字符串)转换成对象
            var worderData = (JSON.parse(e.data));
            if(worderData.resCode == '200') {
                var pointValue = worderData.custInfo.pointValue;
				...
				if(data.resCode == '200' && data.redPacketsList.length > 0) {
					...
					$.each(data.redPacketsList, function(i, item) {
						...
						tpl.push('<div class="swiper-slide hbslide" id="detail' + i + '">' +
							...
							'<img src="' + item.pricture + '?v=2" class="imgclass2" />' +
							'</div>');
						//点击图片跳活动详情
				...
				if(data.resCode == '200' && data.discountList.length > 0) {
					...
					$.each(data.discountList, function(i, item) {
						...
						tpl.push('<div class="swiper-slide zkslide" id="zktz' + i + '"><div class="zkzqon">' +
							...
							'<img src="' + item.picture + '?v=2" class="zk" />' +
							'</div></div>');
						//绑定banner轮播点击
			...
			success: function(data) {
				...
				if(data.resCode == '200' && data.nativeList.length > 0) {
					...
					$.each(data.nativeList, function(i, item) {
						tpl.push('<div class="clic2" id="bd' + i + '"><img src="' + item.picture + '?v=2" class="imgclass3" /></div>');
						//绑定banner轮播点击
	...
	function ywtctx() {
		var transactionId = '1000010070' + timestamp + parseInt(Math.random() * 1000000),
			...
			birth = {
				...
				"columnSource": "1",
				"sign": sign,
				"province": encodeURIComponent(userInfo.province),
				"starLevel": userInfo.userLevel
			};
		$.ajax({
			type: 'GET',
			dataType: "JSON",
			...
			success: function(response) {
				...
				// response =
				// {
				// 	"resCode":"200","resDesc":"SUCCESS","status":"0","type":"7",
                ...
                if (response.resCode == '200') {
                    ...
                    //国庆
                    if (response.type == '7' && response.status == '0') {
                        $('#iframedh').attr("src", "../source/nationalDay/index.html?v=2017");
                        $('#iframedh').show();
                        //满足显示活动，延迟1秒显示勋章墙
                        setTimeout(function () {
                            $('.bigbox').css({
                    ...
                    if (response.type == '1' || response.type == '2' || response.type == '3' || response.type == '4' || response.type == '5' || response.type == '6' || response.type == '7') {
                ...
                if (response.type == undefined || response.type == null || response.type == "" || response.status == "1") {
                    //显示勋章墙
                    $('.bigbox').css({
                        'opacity': '1'
                    })
			...
			success: function(repText) {
				...
				if(parseInt(repText.resCode) == 200 && parseInt(repText.status) == 1) {
					var url = activityUrlList.url + 'businessWin.html?',
						activeParam = {
							"sceneId": repText.sceneId,
							"modelId": repText.modelId,
							"content": repText.content,
							"jumpUrl": repText.jumpUrl,
